...
// ─────────────────────────────────────────────────────────────
// 12. AI TUTOR QUESTION RESPONSE
// ─────────────────────────────────────────────────────────────
const askTutor = async (question, topic, noteContext) => {
    const contextSection = noteContext
        ? `\n\nRELEVANT STUDY MATERIAL:\n"""\n${noteContext}\n"""\nUse this material as primary reference when answering.`
        : '';

    const systemPrompt = `You are SmartNotes AI Tutor — a world-class, patient, and encouraging personal teacher for university students. Your goal is to make every concept crystal-clear.

You must respond with a JSON object in EXACTLY this structure:
{
  "simpleExplanation": "1-2 sentence plain English explanation a 12-year-old could understand",
  "detailedExplanation": "comprehensive 3-5 sentence technical explanation with examples",
  "keyConcepts": ["concept 1", "concept 2", "concept 3"],
  "commonMistakes": ["mistake 1", "mistake 2"],
  "relatedTopics": ["related topic 1", "related topic 2", "related topic 3"],
  "suggestedAction": "one specific study action the student should do next"
}

Respond ONLY with valid JSON. No markdown. No extra text.${contextSection}`;

    const userPrompt = `Student question about "${topic}": ${question}`;

    return await callGroq(systemPrompt, userPrompt, 3000, 0.5);
};

// ─────────────────────────────────────────────────────────────
// 13. TOPIC EXTRACTOR FROM QUESTION
// ─────────────────────────────────────────────────────────────
const extractTopicFromQuestion = async (question) => {
    const systemPrompt = 'You are a topic classifier. Respond with only the academic topic name (1-4 words). No explanation.';
    const userPrompt = `What academic topic is this question about? Question: "${question}"`;
    try {
        const raw = await callGroq(systemPrompt, userPrompt, 100, 0.2);
        return raw?.trim().replace(/[".]/g, '') || 'General';
    } catch {
        return 'General';
    }
};
