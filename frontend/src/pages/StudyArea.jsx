...
const StudyArea = () => {
    ...
    const handleShare = async () => {
        ...
        const shareData = {
            title: `SmartNotes AI: ${activeNote.title}`,
            text: `Check out my notes on "${activeNote.title}" generated by SmartNotes AI!\n\nSummary:\n${activeNote.summary || 'Generated notes.'}`,
        ...
        try {
            ...
            // Don't show error if user just cancelled the native share dialog
            if (err.name !== 'AbortError') {
                setInlineError('Failed to share note.');
            }
        }
    ...
    const handleExportPDF = () => {
        exportPDF(activeNote, setInlineError);
    };
                        ...
                        >
                            ...
                            <div className="h-20 border-b border-slate-100/50 px-8 flex items-center justify-between bg-white/50 sticky top-0 z-30 backdrop-blur-md">
                                ...
                                <div className="flex items-center gap-3">
                                    <button onClick={handleShare} className="btn-secondary py-2 px-4 shadow-none">Share</button>
                ...
                <AnimatePresence>
                    {toastMsg && (
                        <motion.div
                            ...
                            className="fixed bottom-8 left-1/2 z-[100] bg-slate-800 text-white px-6 py-3 rounded-full shadow-2xl font-medium text-sm flex items-center gap-2"
                        >
                            <Sparkles size={16} className="text-secondary" />
                            {toastMsg}
                        </motion.div>
